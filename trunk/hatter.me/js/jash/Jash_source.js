...
function Jash() {
	...
	*/
	this.revision = "$Revision: 1.11 $".replace(/(\$|[A-Za-z]|\s|:)/g,'');
	this.version = "$Name: REL_1_35_4 $".replace(/\$|Name:|\s|REL_/g,'').replace(/_/g,'.');
	this.versionDate = "$Date: 2007/07/13 04:40:03 $";
	...
	*/
	this.main = function() {
		...
		* import stylesheet
		*/
		if(this.returnBrowserType() != "sa") {
			this.stylesheet=document.body.appendChild(document.createElement('link'));
		} else {
			this.stylesheet = document.getElementsByTagName("head")[0].appendChild(document.createElement("link"));
		}
		...
		Jash.Evaluator.prototype = this;		/* inherit Jash scope */
		this.evaluation = new Jash.Evaluator();		/* EVALUATION INSTANCE - use this.evaluation.method() to execute stuff */
