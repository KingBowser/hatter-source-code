...
o.push(m+' */</div>');}
o.push('</div>');}
return o.join('');};return itself;}();(function(){var c=document.cookie,cluster={recommended:['eqeqeq','nomen','undef','white'],goodparts:['bitwise','eqeqeq','nomen','plusplus','undef','white'],clearall:[]},i,input=document.getElementById('input'),n,ns,o,options=['adsafe','bitwise','browser','cap','debug','eqeqeq','evil','forin','fragment','laxbreak','nomen','passfail','plusplus','rhino','undef','white','widget'],output=document.getElementById('output');function getOption(o){var n=document.getElementById(o);return n&&n.checked;}
...
input.onchange=function(e){output.innerHTML='';};ns=document.getElementsByName('jslint');for(i=0;i<ns.length;i+=1){n=ns[i];switch(n.value){case'JSLint':n.onclick=function(e){var b,d=new Date(),j,k='{',oj,op={},v;for(j=0;j<options.length;j+=1){oj=options[j];v=getOption(oj);op[oj]=v;if(b){k+=',';}
k+='"'+oj+'":'+v;b=true;}
...
if(c&&c.length>8){i=c.indexOf('jslint={');if(i>=0){c=c.substring(i+7);i=c.indexOf('}');if(i>1){c=c.substring(0,i+1);o=eval('('+c+')');for(i=0;i<options.length;i+=1){setOption(options[i],o[options[i]]);}}}}
setCluster('recommended');setCluster('goodparts');setCluster('clearall');input.select();})();