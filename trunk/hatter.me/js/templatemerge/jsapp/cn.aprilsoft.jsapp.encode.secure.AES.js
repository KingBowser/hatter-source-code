/*
 * cn.aprilsoft.jsapp.encode.secure.AES.js
 * jsapp, aes functions
 * 
 ...
 */
function Cipher(input, w) {    // main Cipher function [¡ì5.1]
...
function SubBytes(s, Nb) {    // apply SBox to state S [¡ì5.1.1]
  for (var r=0; r<4; r++) {
...
function ShiftRows(s, Nb) {    // shift row r of state S left by r bytes [¡ì5.1.2]
  var t = new Array(4);
  for (var r=1; r<4; r++) {
...
function MixColumns(s, Nb) {   // combine bytes of each col of state S [¡ì5.1.3]
  for (var c=0; c<4; c++) {
...
function AddRoundKey(state, w, rnd, Nb) {  // xor Round Key into state S [¡ì5.1.4]
  for (var r=0; r<4; r++) {
...
function KeyExpansion(key) {  // generate Key Schedule (byte-array Nr+1 x Nb) from Key [¡ì5.2]
  var Nb = 4;            // block size (in words): no of columns in state (fixed at 4 for AES)
...
function AESEncryptCtr(plaintext, password, nBits) {
  ...
  plaintext = encodeUTF8.call(plaintext);
  password = encodeUTF8.call(password);
  ...
  // initialise counter block (NIST SP800-38A ¡ìB.2): millisecond time-stamp for nonce in 1st 8 bytes,
  ...
  ciphertext = encodeBase64.call(ciphertext);  // encode in base64
...
function AESDecryptCtr(ciphertext, password, nBits) {
  var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
  if (!(nBits==128 || nBits==192 || nBits==256)) return '';  // standard allows 128/192/256 bit keys
  ciphertext = decodeBase64.call(ciphertext);
  password = encodeUTF8.call(password);
  //var t = new Date();  // timer
  ...
  plaintext = decodeUTF8.call(plaintext);  // decode from UTF8 back to Unicode multi-byte chars
...
var encodeBase64 = function(utf8encode) {  // http://tools.ietf.org/html/rfc4648
 ...
 */ 
var decodeBase64 = function(utf8decode) {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
  ...
  Class("cn.aprilsoft.jsapp.encode.secure.AES", Extend(), Implement(Encrypt),
  {
    _ENCRYPT_BITS: 256,
    
    encrypt: function(key, value)
    {
      return AESEncryptCtr(value, key, this._ENCRYPT_BITS);
    },
    
    decrypt: function(key, value)
    {
      return AESDecryptCtr(value, key, this._ENCRYPT_BITS);
    }
  });
})();
