...
public class Remove extends BaseAction {
    ...
    protected void doAction(HttpRequest request, HttpResponse response, Map<String, Object> context) {
        ...
        if (StringUtil.isNotEmpty(id)) {
            // remove by id
            ResourceProxyDataAccesObjectInstance.DATA_ACCESS_OBJECT.executeSql(SQL.sql(Cmd.DELETE).table(HttpObject.class).where("id = ?").get(),
                                                                               DBUtil.objects(id));
        } else if ("all".equals(ip)) {
            // remove by id
            ResourceProxyDataAccesObjectInstance.DATA_ACCESS_OBJECT.executeSql(SQL.sql(Cmd.DELETE).table(HttpObject.class).where("access_address = ?").get(),
